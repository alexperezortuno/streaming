...
func serveHlsM3u8(response http.ResponseWriter, request *http.Request, mediaBase, m3u8Name string) {
    base, _ := os.Getwd()
    mediaFile := fmt.Sprintf("%s/web/static/%s/hls/%s", base, mediaBase, m3u8Name)
...
func serveHlsTs(response http.ResponseWriter, request *http.Request, mediaBase, segName string) {
    base, _ := os.Getwd()
    mediaFile := fmt.Sprintf("%s/web/static/%s/hls/%s", base, mediaBase, segName)
    http.ServeFile(response, request, mediaFile)
...
func IndexHandler(w http.ResponseWriter, r *http.Request) {
    base, _ := os.Getwd()
    tmplFile := fmt.Sprintf("%s/web/templates/home.html", base)
...
func UploadApiHandler(response http.ResponseWriter, request *http.Request) {
    ...
    dirName := "/web/static/media/" + rndString
...
func VideoApiHandler(w http.ResponseWriter, r *http.Request) {
    ...
    rowsCount, _ := db.Query("SELECT count(*) FROM video WHERE internal_id=?", id)
...
func ListCreateApiHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
    err := r.ParseForm()
	...
	insert, err := db.Prepare("INSERT INTO list (name) VALUES (?)")
	tools.CheckErr(err, 156)
    ...
    message = []string{"List save successfully!"}
    code = 200
    status = "OK"
    
    response := structs.DefaultResponse{Code: code, Status: status, Message: message}
